    ...
    if (props.apiResultsState === "ready") {
          ...
          <div className='h-full max-h-[750px] max-w-full aspect-[3/5] pl-4 pr-4'>
            <div className='h-[80%] w-full relative'>
              {props.apiResults.slice(1, 2).map((recipe) =>
                <TinderCard className="h-full w-full absolute z-10" swipeRequirementType={'position'} swipeThreshold={100} preventSwipe={['up', 'down', 'left', 'right']} key={props.apiResults} onSwipe={(dir) => swiped(dir)} onCardLeftScreen={() => outOfFrame(props.apiResults)}>
              ...
              {props.apiResults.slice(0, 1).map((recipe) =>
                <TinderCard className="h-full w-full absolute z-10" swipeRequirementType={'position'} swipeThreshold={100} preventSwipe={['up', 'down']} key={props.apiResults} onSwipe={(dir) => swiped(dir, dir)} onCardLeftScreen={() => outOfFrame(props.apiResults)}>
                  <div className='flex justify-center h-full'>
      ...
      return (
        <div className="h-svh w-screen flex flex-col items-center justify-center">
          <div role="status">
            <svg aria-hidden="true" className="w-8 h-8 text-gray-200 animate-spin fill-gunmetal" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <span className='animate-pulse font-outfit font-medium text-lg text-gunmetal'>Loading...</span>
        </div>
      )
    }
    else {
