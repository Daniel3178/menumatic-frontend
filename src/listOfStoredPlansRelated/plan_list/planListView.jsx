...
const PlanListView = (props) => {
  ...
  const renderMeals = (recipesName) => {
    return recipesName.map((recipe, index) => {
      return (<p key={index} className="text-[14px] font-outfit text-whiteSmoke truncate mb-1">{recipe.name}</p>);
    });
  };
  const RecepiePlanOverviewRow = (list) => {
    ...
    return (
      <div className="flex flex-row">
        <div className="bg-cerulean  items-start rounded-2xl mt-4 shadow-xl w-full h-fit hover:brightness-110 hover:shadow-mid"
          ...
          <div className="flex flex-col justify-center ">
            <div className="border-b  border-whiteSmoke flex self-center  pt-4 pb-3 justify-center w-full">
              <div className="flex flex-row w-full">
                <p className="text-[16px] w-[92%] font-outfit  pl-[4%] font-bold text-whiteSmoke flex justify-center">{list.planName}</p>
                <button className="w-[4%] aspect-square bg-cerulean  flex items-center justify-center"
    ...
    return (
      <div className="pt-32 flex flex-col bg-smokeWhite min-h-screen w-screen pr-2 pl-2 lg:pr-80 ">
        <div className="self-center max-w-[840px] w-full">
          ...
          <p className="text-[48px] w-full text-center font-outfit font-bold text-gunmetal tracking-wider">MEAL PLANS</p>
          <div>
            {renderList()}
          </div>
        </div>
